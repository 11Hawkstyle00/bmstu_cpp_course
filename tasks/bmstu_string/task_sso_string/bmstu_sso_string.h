   ...
   public:
    ...
    basic_string(size_t size) {
        ...
        if (is_long_) {
            ...
            std::fill(data_.short_str.buffer, data_.short_str.buffer + size, T(' '));
    ...
    basic_string(const basic_string& other) : is_long_(other.is_long_), size_(other.size_) {
        if (is_long_) {
        ...
        if (len > SSO_CAPACITY) {
            if (!is_long_) {
                ...
                std::copy(data_.short_str.buffer, 
                         data_.short_str.buffer + size_ + 1, 
                         new_ptr);
    ...
    friend basic_string<T> operator+(const basic_string<T>& left,
        ...
        T* dest = result.is_long_ ? result.data_.long_str.ptr : result.data_.short_str.buffer;
        std::copy(left.c_str(), left.c_str() + left.size(), dest);
        std::copy(right.c_str(), right.c_str() + right.size(), dest + left.size());
        dest[total_size] = T();
    ...
    basic_string& operator+=(const basic_string& other) {
        ...
        if (new_size > SSO_CAPACITY) {
            ...
            if (is_long_) {
                ...
                delete[] data_.long_str.ptr;
                is_long_ = false;
                std::copy(buffer, buffer + old_size + 1, data_.short_str.buffer);
    ...
    T& operator[](size_t index) noexcept {
        return is_long_ ? data_.long_str.ptr[index] : data_.short_str.buffer[index];
    }
