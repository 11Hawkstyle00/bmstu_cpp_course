...
class list
{
   ...
   public:
    ...
    {
        using typename abstract_iterator<
            iterator,
            T,
    ...
    T& front() { return head_.next_node_->value_; }

    const T& front() const { return head_.next_node_->value_; }

    T& back() { return tail_.prev_node_->value_; }

    const T& back() const { return tail_.prev_node_->value_; }
    ...
    friend void swap(list<T>& lhs, list<T>& rhs) noexcept { lhs.swap(rhs); }
